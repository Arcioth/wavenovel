<template>
  <div class="wm-sidebar glass-panel" :class="dockClass">
    ...
    <div class="sidebar-group">
      ...
      <!-- Project Home -->
      <button class="sidebar-btn tooltip-right" :class="(!$root.session.selectedTool) ? 'active' : ''" @click="setTool(null)">
      ...
      <button v-for="plugin in pluginsList" :key="plugin.id"
        class="sidebar-btn tooltip-right" 
        :class="$root.session.selectedTool === plugin.id ? 'active' : ''"
        @click="setTool(plugin.id)">
        <svg viewBox="0 0 24 24"><path :d="getSvg(plugin.icon)" /></svg>
        <span class="tooltiptext">{{ getTooltip(plugin.id) }}</span>
      </button>

      <!-- Slot for extra top-level tools injected via ui-registry -->
      <SlotRenderer name="sidebar-top" />
    </div>
    ...
    <div class="sidebar-group">
      ...
      <!-- Save -->
      <button @click="$root.file_downloadDB" class="sidebar-btn tooltip-right">
      ...
      <!-- Google Drive -->
      <button @click="$root.popup.name = 'GoogleDrive'" class="sidebar-btn tooltip-right">
        <svg viewBox="0 0 24 24"><path d="M7.71,3.5L1.15,15L4.58,21L11.13,9.5M9.73,15L6.3,21H19.42L22.85,15M22.28,14L15.42,2H8.58L8.57,2L15.43,14H22.28Z" /></svg>
        <span class="tooltiptext">{{ this.$root.setlang.toolbar.googledrive }}</span>
      </button>

      <!-- Extensions Manager -->
      <button @click="$root.$data.popup.name = 'ExtensionsPopup'" class="sidebar-btn tooltip-right">
        <svg viewBox="0 0 24 24"><path d="M17 11V7H13V5C13 3.9 12.1 3 11 3C9.9 3 9 3.9 9 5V7H5V11H3C1.9 11 1 11.9 1 13C1 14.1 1.9 15 3 15H5V19H9V21C9 22.1 9.9 23 11 23C12.1 23 13 22.1 13 21V19H17V15H19C20.1 15 21 14.1 21 13C21 11.9 20.1 11 19 11H17M15 17H9V15H7V9H9V7H15V9H17V15H15Z" /></svg>
        <span class="tooltiptext">Extensions</span>
      </button>

      <!-- Settings -->
      <button @click="$root.$data.popup.name = 'SettingsPopup'" class="sidebar-btn tooltip-right">
        ...
        <span class="tooltiptext">{{ this.$root.setlang.settings.title }}</span>
      </button>

      <!-- Close Project -->
      <button @click="$root.closeProject()" class="sidebar-btn close-btn tooltip-right">
        <svg viewBox="0 0 24 24"><path d="M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,16.41 16.41,20 12,20M12,2C6.47,2 2,6.47 2,12C2,17.53 6.47,22 12,22C17.53,22 22,17.53 22,12C22,6.47 17.53,2 12,2M14.59,8L12,10.59L9.41,8L8,9.41L10.59,12L8,14.59L9.41,16L12,13.41L14.59,16L16,14.59L13.41,12L16,9.41L14.59,8Z" /></svg>
        <span class="tooltiptext">{{ this.$root.setlang.toolbar.close }}</span>
      </button>
    </div>
  </div>
</template>
...
<script>
import { getPlugins, getPluginSettings } from '@/plugins/manager.js';
...
export default {
  ...
  computed: {
    ...
    dockSettings() {
      return getPluginSettings('dock_menu', { position: 'left', behavior: 'hover' });
    },
    dockClass() {
      return `wm-sidebar-${this.dockSettings.position || 'left'} wm-sidebar-${this.dockSettings.behavior || 'hover'}`;
    }
  },
  methods: {
    getSvg(icon) {
    ...
    toggleNav() {
      if (this.$root.togglenav) {
        this.$root.navbar = !this.$root.navbar;
      }
    }
  }
}
...
.wm-sidebar {
  ...
  bottom: 0;
  width: 70px;
  background-color: var(--top-toolbar, #1e293b);
  display: flex;
...
/* Behavior variants */
.wm-sidebar-left.wm-sidebar-hover {
  transform: translateX(-55px);
  opacity: 0.6;
}
.wm-sidebar-right.wm-sidebar-hover {
  transform: translateX(55px);
  opacity: 0.6;
}

.wm-sidebar.wm-sidebar-hover:hover {
...
.sidebar-btn {
  ...
  align-items: center;
  color: var(--top-toolbar-f, rgba(255,255,255,0.7));
  cursor: pointer;
...
.sidebar-btn:hover {
  ...
  color: var(--button-hover-f, #fff);
  transform: translateX(2px);
}

.sidebar-btn.active {
  background: var(--button-hover, rgba(99, 102, 241, 0.8));
  color: var(--button-hover-f, #fff);
...
.logo-btn:hover .wavemaker_logo-bottom {
  fill: var(--button-hover-f, #fff);
}
